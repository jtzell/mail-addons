...
from flectra.addons.base.models.ir_mail_server import encode_rfc2822_address_header
...
def decode_msg_id(encoded_encrypted_id, env):
    ...
    try:
        ...
        encrypted = base64.b32decode(
            encoded_encrypted_id.encode("utf-8").upper())
    except binascii.Error:
        # Fall back to base64, which was used by the previous versions.
        # This can be removed in Flectra 14.
        try:
            encrypted = base64.urlsafe_b64decode(
                encoded_encrypted_id.encode("utf-8"))
        except binascii.Error:
            _logger.error(
...
class MailServer(models.Model):
    ...
    reply_to_method = fields.Selection(
        [("default", "Flectra Default"),
         ("alias", "Alias"), ("msg_id", "Message ID")],
        "Reply-To Method",
        default="default",
        help="Flectra Default: Don't add any unique identifiers into the\n"
        "Reply-To address.\n"
        "\n"
        ...
        "flectra@mycompany.fi would become flectra+adf9bacd98732@mycompany.fi\n"
        "\n"
        "Note that this method has a flaw: if the headers have dropped\n"
        "and Flectra can't connect the reply to any message in the thread,\n"
        ...
        "address to that Flectra will be able to directly connect the\n"
    ...
    prioritize_reply_to_over_msgid = fields.Boolean(
        ...
        "over the email headers (default Flectra behavior) in incoming "
    ...
    def _get_reply_to_address(self, alias, original_from_name):
        ...
        if self.force_email_reply_to_name:
            reply_to = formataddr(
                (self.force_email_reply_to_name, reply_to_addr))
    ...
    def _compute_headers_example(self):
        for server in self:
            ...
            if server.force_email_reply_to:
                example.append(
                    "Reply-To: {}".format(server.force_email_reply_to))
                ...
                example.append("Reply-To: Flectra default")

            if server.force_email_from:
                example.append(
                    "From: {}".format(
                        formataddr(("Original From Person",
                                   server.force_email_from))
                    )
                )
            else:
                example.append("From: Flectra default")
    ...
    def send_email(
        ...
        # Note that Flectra already has the ability to use a fixed From address
        # by settings "email_from" in the Flectra settings. This is however a
        ...
        if mail_server.force_email_from:
            ...
            message["From"] = formataddr(
                (original_from_name, force_email_from))
            ...
            if mail_server.reply_to_method == "alias":
                ...
                if odoo_msg_id:
                    ...
                    # model set and only pick the first record. Flectra does
                    ...
                    reply_to = mail_server._get_reply_to_address(
                        "{}+{}{}".format(
                            force_email_from.split(
                                "@")[0], MESSAGE_PREFIX, encrypted_id
                        ),
                        original_from_name,
                    )
        ...
        alias = Alias.create(
            {
                ...
                "alias_name": "{from_address}+{random_string}".format(
                    from_address=from_address[0], random_string=random_string(
                        8)
                ),
                "alias_force_thread_id": record_id,
...
class MailThread(models.AbstractModel):
    ...
    def message_parse(self, message, save_original=False):
        ...
        # Remove the "24," part when migrating to Flectra 14.
        prefix_in_to = email_to_localpart and re.search(
            r".*" + MESSAGE_PREFIX +
            "(?P<odoo_id>.{24,32}$)", email_to_localpart
        )
        ...
        if prefix_in_to and prioritize_replyto_over_headers:
            ...
            try:
                ...
                _logger.warning(
                    ...
                    "decrypted. Falling back to default Flectra behavior."
                )
        ...
        # When Flectra compares message_id to the one stored in the database when determining
        #  whether or not the incoming message is a reply to another one, the message_id search
        #  parameter is stripped before the search. But Flectra does not do anything of the sort when
        ...
        #  But when trying to find this message, Flectra takes the above message_id and strips it,
        #  which results in:
        #   '<AM6PR05MB4933DE6BCAD68A037185EBCFFBAF0@AM6PR05MB4933.eurprd05.prod.outlook.com>'
        #  And then the search is done for an exact match, which will fail.
        #
        # Flectra doesn't, so we must strip the message_ids before they are stored in the database
    ...
    def message_route(
        ...
        # NOTE! If you're going to backport this module to Flectra 11 or Flectra 10,
        # you will have to create the mail_bounce_catchall email template
        # because it was introduced only in Flectra 12.
        ...
        try:
            ...
            # Flectra have disappeared at some point and the message was sent to
            ...
            # Flectra to send a bounce message to the sender who sent the email to
